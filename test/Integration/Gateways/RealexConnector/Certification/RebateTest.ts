...
test("JAVA_Rebate_Sample", async (t) => {
  ...
  // request
  const response = await saleResponse.refund(1)
      .withCurrency("EUR")
...
test("JAVA_Rebate_012b", async (t) => {
  ...
  // build transaction
  const saleResponse = await card.charge(1.005)
      .withCurrency("EUR")
      .execute();
  t.truthy(saleResponse);
  t.is("00", saleResponse.responseCode);
  ...
  // request
  const response = await saleResponse.refund(1.005)
      .withCurrency("EUR")
      .withDescription("JAVA-Rebate")
      .execute();
  t.truthy(response);
...
test("JAVA_Rebate_012e", async (t) => {
  ...
  // build transaction
  const saleResponse = await card.charge(100000)
      .withCurrency("EUR")
      .execute();
  t.truthy(saleResponse);
  t.is("00", saleResponse.responseCode);
  ...
  // request
  const response = await saleResponse.refund(100000)
      .withCurrency("EUR")
      .withDescription("JAVA-Rebate")
      .execute();
  t.truthy(response);
